...
const TRAY_ICON_ID: &str = "coral-companion-tray";
const TRAY_OPEN_MENU_ID: &str = "tray-open-main-window";
const TRAY_QUIT_MENU_ID: &str = "tray-quit-app";
const STARTUP_SCRIPT_NAME: &str = "CoralCompanion.vbs";
...
pub fn setup_background<R: Runtime>(app: &AppHandle<R>) -> Result<(), String> {
    let icon = match app_icon::load_coral_companion_icon() {
        Ok(icon) => icon,
        Err(custom_icon_error) => app
            ...
            .ok_or_else(|| {
                format!(
                    "Failed to load the CoralCompanion tray icon ({custom_icon_error}) and the default application icon is missing."
                )
            })?,
    };

    let menu = MenuBuilder::new(app)
        .text(TRAY_OPEN_MENU_ID, "Open CoralCompanion")
        .separator()
